...
import static linqava.Linq.二;
import static linqava.Linq.ᐸ;
import static linqava.Linq.ᐸ二;
import static linqava.Linq.ᐳ;
import static linqava.Linq.ᐳ二;
...
/**
 * Verifies that the HQL produced by {@link Q#getHql()} is correct for each complex HQL/JPQL query.
 *
 * <p>Mapping rules:</p>
 * <ul>
 *   <li>Keywords {@code SELECT / FROM / WHERE / JOIN / ...} are methods; multi-word keywords are a
 *       single method whose words are joined by {@code ‿} (U+203F), e.g. {@code LEFT‿JOIN},
 *       {@code GROUP‿BY}, {@code ORDER‿BY}, {@code UNION‿ALL}.</li>
 *   <li>Columns are getter method references ({@code User::Name}); as a value they are wrapped with
 *       {@code c(...)}, derived/aliased columns use {@code c("alias")}.</li>
 *   <li>The {@code AS} keyword aliases both fields ({@code c(User::id).AS("id")}) and tables
 *       ({@code FROM(User.class).AS("u")}).</li>
 *   <li>Operators are methods on the condition context {@code $}: {@code 二} (=), {@code ᐸ} (&lt;),
 *       {@code ᐳ} (&gt;), {@code ᐸ二} (&lt;=), {@code ᐳ二} (&gt;=), {@code ᐸᐳ} (&lt;&gt;); math:
 *       {@code ᐩ} (+), {@code ｰ} (-), {@code ᚷ} (*), {@code ノ} (/).</li>
 * </ul>
 */
public class QueryTest {
	...
	public void testCteSimple() {
		Q q = WITH("activeUsers",
				SELECT(c(User::id).AS("id"), c(User::name).AS("name"))
						.FROM(User.class).AS("u")
						.WHERE($ -> $.二(User::active, true)))
				.SELECT(c("a.name"))
	...
	// WITH recentOrders AS (...), bigSpenders AS (...) SELECT ... FROM bigSpenders b JOIN Customer c ...
	@Test
	public void testCteMultiple() {
		Q q = WITH("recentOrders",
				SELECT(c(Order::id).AS("id"), c(Order::customerId).AS("customerId"))
						.FROM(Order.class).AS("o")
						.WHERE($ -> $.ᐳ(Order::createdAt, param("since"))))
				.WITH("bigSpenders",
						SELECT(c(Order::customerId).AS("customerId"), SUM(Order::total).AS("total"))
								.FROM("recentOrders").AS("r")
								.JOIN(Order.class).AS("o").ON($ -> $.二(Order::id, c("r.id")))
								.GROUP‿BY(c(Order::customerId))
								.HAVING($ -> $.ᐳ(SUM(Order::total), param("threshold"))))
				.SELECT(Customer::name, c("b.total"))
				.FROM("bigSpenders").AS("b")
				.JOIN(Customer.class).AS("c").ON($ -> $.二(Customer::id, c("b.customerId")))
				.ORDER‿BY(c("b.total").DESC());
	...
	// WITH RECURSIVE empHierarchy AS (anchor UNION ALL recursive) SELECT h.id, h.depth ...
	@Test
	public void testRecursiveCte() {
		Q q = WITH‿RECURSIVE("empHierarchy",
				SELECT(c(Employee::id).AS("id"), c(Employee::managerId).AS("managerId"), lit(0).AS("depth"))
						.FROM(Employee.class).AS("e")
						.WHERE($ -> $.IS‿NULL(Employee::managerId))
						.UNION‿ALL(
								SELECT(c(Employee::id).AS("id"), c(Employee::managerId).AS("managerId"),
										c("h.depth").ᐩ(1).AS("depth"))
										.FROM(Employee.class).AS("e")
										.JOIN("empHierarchy").AS("h").ON($ -> $.二(Employee::managerId, c("h.id")))))
				.SELECT(c("h.id"), c("h.depth"))
				.FROM("empHierarchy").AS("h")
	...
	// SELECT o FROM Order o WHERE o.total = (SELECT MAX(o2.total) FROM Order o2 WHERE o2.customerId = o.customerId)
	@Test
	public void testCorrelatedSubquery() {
		Q q = SELECT(Order.class)
				.FROM(Order.class).AS("o")
				.WHERE($ -> $.二(Order::total,
						SELECT(MAX(Order::total))
								.FROM(Order.class).AS("o2")
								.WHERE(s -> s.二(Order::customerId, c("o", Order::customerId)))));
	...
	// SELECT c FROM Customer c WHERE EXISTS (SELECT 1 FROM Order o WHERE o.customerId = c.id AND o.status = 'PAID')
	@Test
	public void testExistsSubquery() {
		Q q = SELECT(Customer.class)
				.FROM(Customer.class).AS("c")
				.WHERE($ -> $.EXISTS(
						SELECT(lit(1))
								.FROM(Order.class).AS("o")
								.WHERE(s -> s.二(Order::customerId, c("c", Customer::id)).AND().二(Order::status, "PAID"))));
	...
	// SELECT p FROM Product p WHERE p.categoryId IN (SELECT cat.id FROM Category cat WHERE cat.parentId = :parent)
	@Test
	public void testInSubquery() {
		Q q = SELECT(Product.class)
				.FROM(Product.class).AS("p")
				.WHERE($ -> $.IN(Product::categoryId,
						SELECT(Category::id)
								.FROM(Category.class).AS("cat")
								.WHERE(s -> s.二(Category::parentId, param("parent")))));
	...
	// SELECT o.customerId, COUNT(o.id), SUM(o.total) FROM Order o WHERE o.status <> 'CANCELLED' GROUP BY ... HAVING ... ORDER BY ...
	@Test
	public void testGroupByHaving() {
		Q q = SELECT(Order::customerId, COUNT(Order::id), SUM(Order::total))
				.FROM(Order.class).AS("o")
				.WHERE($ -> $.ᐸᐳ(Order::status, "CANCELLED"))
				.GROUP‿BY(c(Order::customerId))
	...
	// SELECT DISTINCT c FROM Customer c LEFT JOIN FETCH c.orders o LEFT JOIN FETCH o.items WHERE c.country = :country
	@Test
	public void testJoinFetch() {
		Q q = SELECT(DISTINCT(Customer.class))
				.FROM(Customer.class).AS("c")
				.LEFT‿JOIN‿FETCH(c(Customer::orders)).AS("o")
				.LEFT‿JOIN‿FETCH(c("o.items"))
				.WHERE($ -> $.二(Customer::country, param("country")));
	...
	// SELECT o.id, c.name, p.title FROM Order o JOIN Customer c ON ... JOIN OrderItem oi ON ... JOIN Product p ON ...
	@Test
	public void testMultipleJoinsWithOn() {
		Q q = SELECT(Order::id, c(Customer::name), c(Product::title))
				.FROM(Order.class).AS("o")
				.JOIN(Customer.class).AS("c").ON($ -> $.二(Customer::id, c("o", Order::customerId)))
				.JOIN(OrderItem.class).AS("oi").ON($ -> $.二(OrderItem::orderId, c("o", Order::id)))
				.JOIN(Product.class).AS("p").ON($ -> $.二(Product::id, c("oi", OrderItem::productId)))
	...
	// SELECT o.customerId, o.id, o.total, ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ... DESC) AS rn FROM Order o
	@Test
	public void testWindowFunction() {
		Q q = SELECT(Order::customerId, c(Order::id), c(Order::total),
				ROW_NUMBER().OVER(
						PARTITION‿BY(c(Order::customerId)).ORDER‿BY(Order::total).DESC()).AS("rn"))
				.FROM(Order.class).AS("o");
		assertEquals("select o.customerId, o.id, o.total, "
				+ "row_number() over (partition by o.customerId order by o.total desc) as rn from Order o", q.getHql());
	}

	// SELECT u.email FROM User u WHERE u.active = true UNION ALL SELECT s.email FROM Supplier s WHERE s.preferred = true
	@Test
	public void testUnionAll() {
		Q q = SELECT(c(User::email).AS("contact"))
				.FROM(User.class).AS("u")
				.WHERE($ -> $.二(User::active, true))
				.UNION‿ALL(
						SELECT(c(Supplier::email).AS("contact"))
								.FROM(Supplier.class).AS("s")
								.WHERE(s -> s.二(Supplier::preferred, true)));
	...
	// SELECT NEW CustomerSummary(c.id, c.name, COUNT(o.id)) FROM Customer c LEFT JOIN c.orders o GROUP BY c.id, c.name
	@Test
	public void testConstructorExpression() {
		Q q = SELECT(NEW(CustomerSummary.class, c(Customer::id), c(Customer::name), COUNT(c("o", Order::id))))
				.FROM(Customer.class).AS("c")
				.LEFT‿JOIN(Customer::orders).AS("o")
	...
	// SELECT c FROM Customer c WHERE :product MEMBER OF c.wishlist AND c.orders IS NOT EMPTY AND SIZE(c.orders) > 3
	@Test
	public void testCollectionMemberAndEmpty() {
		Q q = SELECT(Customer.class)
				.FROM(Customer.class).AS("c")
				.WHERE($ -> $.MEMBER‿OF(param("product"), c(Customer::wishlist))
						.AND().IS‿NOT‿EMPTY(Customer::orders)
	...
	// SELECT p FROM Payment p WHERE TREAT(p AS CreditCardPayment).cardType = :cardType OR TREAT(...).iban LIKE :ibanPrefix
	@Test
	public void testTreatPolymorphism() {
		Q q = SELECT(Payment.class)
				.FROM(Payment.class).AS("p")
				.WHERE($ -> $.二(TREAT(Payment.class, CreditCardPayment.class).dot(CreditCardPayment::cardType),
						param("cardType"))
	...
	// WITH rankedOrders AS (... RANK() OVER (...) AS rnk ...) SELECT r.customerId, r.id, r.total FROM rankedOrders r WHERE r.rnk = 1
	@Test
	public void testCteWithWindowFunction() {
		Q q = WITH("rankedOrders",
				SELECT(c(Order::id).AS("id"), c(Order::customerId).AS("customerId"), c(Order::total).AS("total"),
						RANK().OVER(
								PARTITION‿BY(c(Order::customerId)).ORDER‿BY(Order::total).DESC()).AS("rnk"))
						.FROM(Order.class).AS("o"))
				.SELECT(c("r.customerId"), c("r.id"), c("r.total"))
	...
	// SELECT c.name, (SELECT COUNT(o.id) FROM Order o WHERE o.customerId = c.id) AS orderCount FROM Customer c ORDER BY orderCount DESC
	@Test
	public void testScalarSubqueryInSelect() {
		Q q = SELECT(Customer::name,
				sub(SELECT(COUNT(Order::id))
						.FROM(Order.class).AS("o")
						.WHERE(s -> s.二(Order::customerId, c("c", Customer::id)))).AS("orderCount"))
				.FROM(Customer.class).AS("c")
				.ORDER‿BY(c("orderCount").DESC());
	...
	// SELECT o.customerId, SUM(CASE WHEN ... END) AS paidTotal, COALESCE(AVG(NULLIF(...)), 0) AS avgDiscount FROM Order o GROUP BY ...
	@Test
	public void testNestedCaseInAggregateWithCoalesce() {
		Q q = SELECT(Order::customerId,
				SUM(CASE().WHEN($ -> $.二(Order::status, "PAID")).THEN(c(Order::total)).ELSE(0).END()).AS("paidTotal"),
				COALESCE(AVG(NULLIF(Order::discount, 0)), 0).AS("avgDiscount"))
				.FROM(Order.class).AS("o")
				.GROUP‿BY(c(Order::customerId));
		assertEquals("select o.customerId, sum(case when o.status = 'PAID' then o.total else 0 end) as paidTotal, "
				+ "coalesce(avg(nullif(o.discount, 0)), 0) as avgDiscount from Order o group by o.customerId", q.getHql());
	}

	// SELECT o FROM Order o WHERE status = 'PAID' AND (total > 100 OR (discount < 5 AND (customerId = 42 OR (total >= 1000 AND discount <= 50))))
	@Test
				...
				.WHERE(AND(
						...
						OR(
								...
								AND(
										ᐸ(Order::discount, 5),
										OR(
												二(Order::customerId, 42),
												AND(
														ᐳ二(Order::total, 1000),
														ᐸ二(Order::discount, 50)))))));
		assertEquals("select o from Order o where (o.status = 'PAID' and (o.total > 100 or "
				+ "(o.discount < 5 and (o.customerId = 42 or (o.total >= 1000 and o.discount <= 50)))))", q.getHql());
	}
	...
	/** A minimal {@link EntityManager} that records the HQL passed to {@code createQuery} and returns a fixed list. */
	private static EntityManager fakeEntityManager(String[] capturedHql, List<?> resultList) {
		...
		InvocationHandler queryHandler = (proxy, method, args) ->
				method.getName().equals("getResultList") ? resultList : proxy;
